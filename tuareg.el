...
(defun tuareg-install-font-lock ()
  (setq
   ...
   `(,@(and (tuareg-editing-ls3)
     ...
     ("\\<\\(as\\|do\\(ne\\|wnto\\)?\\|else\\|for\\(_lwt\\)?\\|if\\|m\\(atch\\|utable\\)\\|new\\|p\\(arser\\|rivate\\)\\|t\\(hen\\|o\\|ry\\(_lwt\\)?\\)\\|w\\(h\\(en\\|ile\\)\\|ith\\)\\|lazy\\|exception\\|raise\\|failwith\\|exit\\|assert\\|fun\\(ction\\)?\\|finally\\)\\>"
...
(defconst tuareg-block-regexp
  (concat (tuareg-ro "for" "for_lwt" "while" "do" "if" "begin" "sig" "struct" "object")
          "\\|[][(){}]\\|\\*)"))
...
(defconst tuareg-binding-regexp "\\(\\<and\\>\\|(*\\<l\\(e\\|w\\)t\\>\\)")
...
(defconst tuareg-find-and-match-regexp
  (concat (tuareg-ro "do" "done" "else" "end" "in" "then" "down" "downto"
                     "for" "for_lwt" "while" "do" "if" "begin" "sig" "struct" "class"
...
Gathered here for memoization and dynamic reconfiguration purposes."
  (setq
   tuareg-find-comma-match-regexp
    (tuareg-make-find-kwop-regexp
     (concat (tuareg-ro "and" "match" "begin" "else" "exception" "then" "try"
                        "try_lwt" "with" "or" "fun" "function" "let" "lwt" "do")
             "\\|->\\|[[{(]"))
   tuareg-find-with-match-regexp
    (tuareg-make-find-kwop-regexp
     (concat (tuareg-ro "match" "try" "try_lwt" "module" "begin" "with" "type")
             "\\|[[{(]"))
   ...
   tuareg-find-semicolon-match-regexp
    (tuareg-make-find-kwop-regexp
     (concat ";" tuareg-no-more-code-this-line-regexp "\\|->\\|"
             (tuareg-ro "let" "lwt" "method" "with" "try" "try_lwt" "initializer")))
...
(defconst tuareg-phrase-regexp-3
  (tuareg-ro "and" "end" "every" "in" "with" "finally"))
...
(defconst tuareg-paren-or-indentation-stop-regexp
  (tuareg-ro "and" "do" "in" "with" "finally"))
...
(defconst tuareg-=-indent-regexp-1
  (tuareg-ro "val" "let" "lwt" "method" "module" "class" "when" "for" "if" "do"))
...
(defun tuareg-compute-kwop-indent (kwop)
  ...
  (let* ((old-point (point))
    ...
    (cond ((string= kwop "|")
          ...
          ((string= kwop "in")
           (+ (current-column)
              (tuareg-add-default-indent (or (string= matching-kwop "let") (string= matching-kwop "lwt")))))
...
(defun tuareg-compute-indent ()
  (save-excursion
    (cond
     ...
     ((tuareg-in-literal-p)
      (current-column))
     ((or (looking-at "\\<l\\(e\\|w\\)t\\>") (looking-at "\\<open\\>"))
      (if (tuareg-looking-at-internal-let)
